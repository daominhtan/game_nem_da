...
function deg(d) { return d * RAD; }
function shortenHex(c) {
    if (c.length === 7)
        return c;
    return c;
}
...
function drawLimb(ctx, x, y, angle, length, width, color, outline) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
...
function drawBody(ctx, app, pose, flip) {
    const s = 1;
    const cx = FW / 2;
...
function drawHair(ctx, app, rx, ry) {
...
function defaultPose(overrides) {
    return {
...
export class CharacterGenerator {
    static generateCharacter(scene, charId, appearance) {
        ...
        for (const animKey of ANIMATION_KEYS) {
            ...
            if (scene.anims.exists(`${texKey}_${animKey}`))
                continue;
            scene.anims.create({
