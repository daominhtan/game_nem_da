...
export default class PlayerEntity {
    ...
    update(time, delta) {
        ...
        if (this.statusEffect === 'stunned') {
            ...
            this.sprite.y -= 1;
            if (this.sprite.y < this.targetY - 5)
                this.sprite.y += 2;
        }
        else {
    ...
    updateStatusTimerDisplay(seconds) {
        const scene = this.sprite.scene;
        if (seconds <= 0) {
            if (this.statusTimerText) {
                this.statusTimerText.destroy();
    ...
    startZZZ() {
        ...
        this.zzzTimer = scene.time.addEvent({
            ...
            loop: true,
            callback: () => {
                const zzz = scene.add.text(this.sprite.x + Phaser.Math.Between(-10, 10), this.sprite.y - 60, 'Z', { fontSize: `${Phaser.Math.Between(14, 24)}px`, color: '#cc88ff',
                    stroke: '#000', strokeThickness: 2 }).setOrigin(0.5).setDepth(101);
